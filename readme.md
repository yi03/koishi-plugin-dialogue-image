...
5.  **`imgqa.query` (别名: `查询问答`, `imgquery`, `查问答`)**
    *   **功能**: 根据问答 ID 查询并显示其对应的 **问题** 图片，或通过 **回复** 一张图片查询其对应的 **所有回答** 列表。
    *   **语法**:
        *   查询问题图片 (通过 ID): `imgquery [-g] <ID>`
        *   查询回答列表 (通过图片): **回复** 一张你想查询的图片，然后输入 `imgquery [-g]`
    *   **选项**:
        *   `-g, --global`: 查询所有可访问范围的问答记录（需要权限 3）。
    *   **用法**:
        *   要查找 ID 为 123 的问答对应的问题图片：`imgquery 123`
        *   要查找某张图片设置了哪些回答：先回复那张图片，然后发送命令 `imgquery`
    *   **查找范围**: 默认在群聊中会查找本群和全局记录，私聊中只查找全局记录。使用 `-g` 选项可以查找所有范围的问答记录（需要权限 3）。
    *   **输出**:
        *   查询 ID 时: 发送一条包含问答信息（ID、范围、哈希前缀）和 **问题图片** 的消息。
        *   查询图片时: 发送一条消息，列出与该图片哈希匹配的所有 **回答记录**，包含它们的 ID、范围和回答内容预览。
