...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.command(`${name}.add [...answerElements:el]`, '添加图片问答', { authority: 1 })
    ...
    .action(async ({ session, options }, answerElements: h[]) => {
      ...
      try {
        ...
        if (exactMatch) { // 如果存在完全相同的问答
          if (exactMatch.probability === probability) { // 且概率也相同
    ...
    .action(async ({ session, options }) => {
      ...
      try {
        ...
        const list = await ctx.database.get(TABLE_NAME, query, {
          ...
          sort: { guildId: 'asc', imageHash: 'asc', id: 'asc' } // 排序以获得稳定结果
        });
        ...
        const output = list.map(item => {
          let displayAnswer = item.answer.replace(/<image.*?>/g, '[图片]'); // 回答预览
  ...
  ctx.command(`${name}.delete <IDs:string>`, '删除指定ID的图片问答')
    ...
    .usage( // (未修改)
      h.normalize([
        ...
        h.text("用法：imgdel "), h.text("<ID1,ID2,...>"), h.text(" [-g]"),
      ]).join('')
    )
  ...
  ctx.command(`${name}.delete.all <hashPrefix:string>`, '删除指定问题哈希(或前缀)对应的所有问答')
    ...
    .usage( // (未修改)
      h.normalize([
        ...
        h.text("用法: imgdelall "), h.text("<哈希或前缀>"), h.text(" [-g]"),
      ]).join('')
    )
    .action(async ({ session, options }, hashPrefix) => {
      ...
      try {
        ...
        if (distinctHashes.length === 0) { // 未找到匹配项
  ...
  ctx.command(`${name}.query <ID:natural>`, '查询问答ID对应的问题图片')
    ...
    .action(async ({ session }, id) => {
      ...
      try {
        ...
        try {
          await fs.access(localImagePath); // 检查文件是否存在且可访问
          ...
          if (fileError.code === 'ENOENT') { // 文件未找到
  ...
  ctx.command(`${name}.clear`, '清理本地存储中未被引用的图片', { authority: 3 })
    ...
    .action(async ({ session, options }) => {
      ...
      let filesOnDisk: string[] = []; // 存储磁盘上的所有文件名
      let deletedCount = 0; // 成功删除的文件数
      let failedToDelete: { filename: string; error: string }[] = []; // 删除失败的文件列表
      let skippedCount = 0; // 跳过的非图片文件或目录数

      try {
        // 阶段 1: 分批扫描数据库，收集引用的文件名
            ...
            if (record.answer) {
              try {
                ...
                h.select(answerElements, 'img').forEach(img => {
                  ...
                  if (src?.startsWith('file://')) { // 只处理本地文件 URI
                    try {
        ...
        try {
          ...
          if (readDirError.code === 'ENOENT') { // 目录不存在
        ...
        for (const filename of filesToDelete) {
          ...
          try {
            ...
            // 跳过目录和非已知图片扩展名的文件
            if (!stats.isFile()) { skippedCount++; continue; }
            const ext = path.extname(filename).toLowerCase();
            if (!KNOWN_IMAGE_EXTENSIONS.includes(ext)) { skippedCount++; continue; }
  ...
  ctx.command(`${name}.modify <id:natural> [...newAnswerContent:text]`, '修改指定ID的图片问答')
    ...
    .usage( // (未修改)
      h.normalize([
        ...
        h.text("用法: imgmod "), h.text("<ID>"), h.text(" [新的回答内容...] [-p 新概率]\n"),
    ...
    .action(async ({ session, options }, id, newAnswerContent) => {
      ...
      try {
        ...
        let actualAnswerUpdate = false; // 标记是否真的需要更新回答
        let actualProbabilityUpdate = false; // 标记是否真的需要更新概率
